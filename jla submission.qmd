...
The data were collected from the Czech Ministry of Transport Open Data @registr monthly snapshots. For the purpose of our analysis only registrations from years `r min(rocne$rok_registrace)` to `r max(rocne$rok_registrace)` were used. 
...
In order to model consumer activity we focused on retail registrations. Electrification of corporate fleets is expected to contribute materially to the overall reduction of transport CO~2~ emissions the behavior of corporate fleets, but it is best understood as a separate problem, impractical to model based on local socioeconomic variables.

For a deeper illustration consider comparison of Retail vs. Non-Retail EV penetration (@fig-retail-nonretail), noting the high penetration of EV registrations in Mladá Boleslav and Vrchlabí; these can be explained better as location of facilities of Škoda Auto than as output of local socioeconomic factors.
...
For the socioeconomic predictors we used results of the previous (2011) census @vysledky11; the results of the 2021 census were not available at the time of writing in necessary level of detail. The census data were accessed from the Statistical Office API using *czso* package @bouchal22.

Data processing and modelling was done in statistical programming language R @erko. Modelling was performed using package *leaps* @miller20 for stepwise regression. Bayesian information criterion @schwarz78 was used as primary metric for variable selection.

The vehicle registration dataset provides spatial breakdown to the level of municipalities with extended powers (ORP). Given that the population of the `r nrow(RCzechia::orp_polygony())` Czech ORPs varies by several orders of magnitude (from more than a million in Prague to less than 10 thousands in Pacov or Králíky) it was impractical to model registrations as absolute figures. 
...
Spatial information regarding the ORPs was obtained using package *RCzechia* @lacko23. Spatial autocorrelation of EV penetration at ORP level was measured using Moran's I statistic, as defined in @bivand_pebesma23. 
...
The value of the Moran's I statistic of Retail EV penetration was calculated via package *sfdep*  @parry23, using rook neighborhood definition for determining the adjacency matrix of individual ORPs. 
...
```
The original data of retail EV penetration showed high degree of spatial autocorrelation as calculated using `r orig_moran$method`. The observed value of Moran's I statistic for EV penetration was `r orig_moran$statistic`, indicating a significant autocorrelation at p-value `r orig_moran$p.value`.

The same statistic calculated for residuals after linear modelling for the `r length(names(coef(leaps_model, which.min(summary(leaps_model)$bic)))[-1])` socioeconomic predictors showed much less spatial dependence, with Moran's I at `r resids_moran$statistic`. This implies p-value `r resids_moran$p.value`, leading us to reject a hypothesis of spatial autocorrelation of model residuals. 
...
The coefficients identified via stepwise selection in @tbl-coeffs can be split into two groups: the first related to education - with the share of population with tertiary education leading to higher EV penetration, and share of population with secondary education only leading to lower EV penetration.

The second group relates to age structure: share of population in early productive age (20–29 years) drives EV adoption down, and share of population in middle to upper middle productive age (30–39 and 40–49) drives EV adoption up.

Other socioeconomic factors, including those related to employment status, nationality and religion, did not have a significant effect on EV adoption as measured by the BIC criterion.
