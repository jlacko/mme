...
```
The original data of retail EV penetration showed high degree of spatial autocorrelation as calculated using `r orig_moran$method`. The observed value of Moran's I statistic for EV penetration was `r prettynum(orig_moran$estimate[1])`, with Z-score `r prettynum(orig_moran$statistic)` indicating a significant autocorrelation at *p*-value `r orig_moran$p.value`.
